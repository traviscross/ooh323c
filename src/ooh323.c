...
int ooOnReceivedSetup(ooCallData *call, Q931Message *q931Msg)
{
   ...
   if(pDisplayIE)
   {
      call->remoteDisplayName = (ASN1OCTET*) ASN1MALLOC(call->pctxt,
                                 pDisplayIE->length*sizeof(ASN1OCTET));
      memset(call->remoteDisplayName, 0, pDisplayIE->length*sizeof(ASN1OCTET));
...
int ooOnReceivedSignalConnect(ooCallData* call, Q931Message *q931Msg)
{
   ...
   if(connect->m.fastStartPresent)
   {
      ...
      for(i=0; i<(int)connect->fastStart.n; i++)
      {
         ...
         if(!olc)
         {
            OOTRACEERR3("ERROR:Allocating memory for OLC, in received faststart"
                        "CONNECT message (%s, %s)\n", call->callType,
                        call->callToken);
            /*Mark call for clearing */
            if(call->callState < OO_CALL_CLEAR)
         ...
         memset(olc, 0, sizeof(H245OpenLogicalChannel));
         memcpy(msgbuf, connect->fastStart.elem[i].data, connect->fastStart.elem[i].numocts);
         ...
         if(!strcmp(pChannel->dir, "transmit"))
         {
            if(olc->reverseLogicalChannelParameters.multiplexParameters.t !=
               T_H245OpenLogicalChannel_reverseLogicalChannelParameters_multiplexParameters_h2250LogicalChannelParameters)
                        {
               OOTRACEERR4("ERROR:Unknown multiplex parameter type for channel %d "
                           "(%s, %s)\n", olc->forwardLogicalChannelNumber,
                           call->callType, call->callToken);
               continue;
                        }
   ...
   if(connect->m.h245AddressPresent)
   {
      if(call->isTunnelingActive)
      {
         ...
         OOTRACEINFO3("Tunneling is disabled for call as H245 address is provided "
                      "in connect message (%s, %s)\n", call->callType,
                       call->callToken);
      }
         ...
         if(pNode)
         {
            ...
            if(pAliasAddress)
            {
               ...
               case T_H225AliasAddress_h323_ID:
                  ...
                  newAlias->value = (char*)ASN1MALLOC(call->pctxt,
                             (pAliasAddress->u.h323_ID.nchars+1)*sizeof(char));
                  memset(newAlias->value, 0,
                             (pAliasAddress->u.h323_ID.nchars+1)*sizeof(char));
               ...
               case T_H225AliasAddress_transportID:
                  ...
                  /* hopefully ip:port value can't exceed more than 30 characters */
                  newAlias->value = (char*)ASN1MALLOC(call->pctxt, 30*sizeof(char));
